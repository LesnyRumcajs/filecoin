...
const ACTORS: &[(&str, &str)] = &[
    ("BASIC_NFT_ACTOR_WASM", "basic_nft_actor"),
    ("BASIC_RECEIVER_ACTOR_WASM", "basic_receiving_actor"),
    ("BASIC_TOKEN_ACTOR_WASM", "basic_token_actor"),
    ("BASIC_TRANSFER_ACTOR_WASM", "basic_transfer_actor"),
    ("FRC46_TEST_ACTOR_WASM", "frc46_test_actor"),
    ("FRC53_TEST_ACTOR_WASM", "frc53_test_actor"),
];
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for (var, pkg) in ACTORS {
        let bin = bundle_dir.join(WASM_TARGET).join("wasm").join(format!("{pkg}.wasm"));
        writeln!(&mut wasm_bin_file, "pub const {var}: &[u8] = include_bytes!({bin:?});")
